...
const JSON_PATH = path.join(process.cwd(), "data", "eventos-rutas-mototurismo-espana-2026.json");
const BACKUP_DIR = path.join(process.cwd(), "data", "backups");
const VALID_CATEGORIES = new Set([
  "Ruta motera",
  "Mototurismo",
  "Ruta trail",
  "Desafío mototurístico",
  "Ruta solidaria",
  "Ruta nocturna",
  "Touring",
  "Motoalmuerzo",
...
function normalizeVehicleType(value, searchableText) {
  ...
  if (text.includes("moto") || text.includes("mototurismo") || text.includes("motera") || text.includes("trail")) return "moto";
...
function normalizeTags(value, category, reasons) {
  ...
  for (const tag of [...tags, "ruta", "Rutas", category].filter(Boolean)) {
...
function validateEvent(value, index) {
  ...
  const category = getString(value.category) || "Ruta motera";
  const discipline = getString(value.discipline) || "Rutas";
  ...
  if (normalizeText(discipline) !== "rutas") reasons.push('discipline debe ser "Rutas"');
  ...
  if (!includesAny(searchableText, DOMAIN_TERMS)) {
    reasons.push("el evento no parece realmente de rutas, mototurismo o touring");
  }
  ...
  if (reasons.length) {
    return {
      index,
      id: getString(value.id) || `(rutas-${slug || index + 1})`,
      title: title || "(sin titulo)",
      reasons,
    };
  }

  const id = getString(value.id) || `rutas-${slug}`;
...
async function writeBackup(events) {
  ...
  const backupPath = path.join(
    BACKUP_DIR,
    `events-backup-before-rutas-${formatTimestamp(new Date())}.json`,
  );
  const backupContent = JSON.stringify(
    {
      createdAt: new Date().toISOString(),
      source: "public.events",
      reason: "backup before non-destructive rutas import",
      count: events.length,
      events,
    },
    null,
...
async function main() {
  ...
  console.log("Importacion segura de eventos de rutas y mototurismo 2026");
  ...
  if (insertError) {
    console.error("\nError de Supabase al insertar eventos de rutas:");
...
main().catch((error) => {
  ...
  console.error(`\nImportacion de rutas fallida: ${message}`);
  process.exitCode = 1;
});
