...
const SEED_PATH = path.join(process.cwd(), "data", "eventomotor-rallyes-coches-2026-seed-77.json");
...
function sameNormalizedTitle(leftTitle: string, rightTitle: string) {
  return normalizeComparable(leftTitle) === normalizeComparable(rightTitle);
}

function getExistingStart(event: ExistingEvent | ValidSeedEvent) {
  return "start_date" in event ? event.start_date : event.start;
}

function sameStart(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  return seedEvent.start === getExistingStart(existingEvent);
}

function sameCityOrProvince(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  const sameCity =
    normalizeComparable(seedEvent.city) !== "" &&
    normalizeComparable(seedEvent.city) === normalizeComparable(existingEvent.city);
  const sameProvince =
    normalizeComparable(seedEvent.province) !== "" &&
    normalizeComparable(seedEvent.province) === normalizeComparable(existingEvent.province);

  return sameCity || sameProvince;
}

function sameTitleDateAndPlace(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  return sameNormalizedTitle(seedEvent.title, existingEvent.title) && sameStart(seedEvent, existingEvent) && sameCityOrProvince(seedEvent, existingEvent);
}

function sameDateAndProvince(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  return (
    sameStart(seedEvent, existingEvent) &&
    normalizeComparable(seedEvent.province) === normalizeComparable(existingEvent.province)
  );
}

function similarDiscipline(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  if (!("discipline" in existingEvent) || !existingEvent.discipline) {
    return true;
  }

  const left = normalizeComparable(seedEvent.discipline);
  const right = normalizeComparable(existingEvent.discipline);

  return left === right || left.includes(right) || right.includes(left) || titleSimilarity(left, right) >= 0.72;
}

function sameTitleDateAndSourceUrl(seedEvent: ValidSeedEvent, existingEvent: ExistingEvent | ValidSeedEvent) {
  const existingSourceUrl = "source_url" in existingEvent ? existingEvent.source_url : existingEvent.sourceUrl;
  const sourceUrlKey = normalizeSourceUrl(seedEvent.sourceUrl);

  return (
    sourceUrlKey !== "" &&
    sourceUrlKey === normalizeSourceUrl(existingSourceUrl) &&
    sameNormalizedTitle(seedEvent.title, existingEvent.title) &&
    sameStart(seedEvent, existingEvent)
  );
}
...
function normalizeTags(value: unknown, discipline: string, reasons: string[]) {
  ...
  for (const tag of [...tags, "coche", discipline]) {
...
function parseVehicleType(value: RawSeedEvent) {
  return getString(value.vehicleType) || getString(value.vehicle_type);
}
...
function validateSeedEvent(value: unknown, index: number): ValidSeedEvent | InvalidSeedEvent {
  ...
  const vehicleType = parseVehicleType(value);

  if (!vehicleType) {
    reasons.push('missing required field "vehicleType" or "vehicle_type"');
  } else if (vehicleType !== "coche") {
    reasons.push('vehicleType must be "coche" for this import');
  }
  ...
  const tags = normalizeTags(value.tags, discipline || "Rally", reasons);
  ...
  return {
    ...
    slug,
    title,
    championship: getString(value.championship) || "Rallyes y eventos de coches",
    discipline,
    vehicleType: "coche",
    start,
    end,
...
function splitValidEvents(parsedEvents: unknown[]) {
  ...
  parsedEvents.forEach((event, index) => {
    ...
    const existingByTitleDatePlace = validEvents.find((validEvent) => sameTitleDateAndPlace(result, validEvent));
    const existingByTitleDateSourceUrl = validEvents.find((validEvent) => sameTitleDateAndSourceUrl(result, validEvent));
    const similarEvent = validEvents.find((validEvent) => {
      return (
        sameDateAndProvince(result, validEvent) &&
        titleSimilarity(result.title, validEvent.title) >= SIMILAR_TITLE_THRESHOLD &&
        similarDiscipline(result, validEvent)
      );
    });
    ...
    if (existingBySlug) {
      duplicatesInSeed.push({ event: result, matchedEvent: existingBySlug, reason: "mismo slug generado en JSON" });
      return;
    }

    if (existingByTitleDatePlace) {
      duplicatesInSeed.push({
        event: result,
        matchedEvent: existingByTitleDatePlace,
        reason: "mismo title normalizado + fecha + ciudad o provincia en JSON",
      });
      return;
    }

    if (existingByTitleDateSourceUrl) {
      duplicatesInSeed.push({
        event: result,
        matchedEvent: existingByTitleDateSourceUrl,
        reason: "mismo sourceUrl + mismo title normalizado + misma fecha en JSON",
      });
      return;
    }
    ...
    if (similarEvent) {
      duplicatesInSeed.push({
        ...
        reason: "title muy parecido + misma fecha + misma provincia en JSON",
...
async function writeBackup(events: unknown[]) {
  ...
  const backupPath = path.join(
    BACKUP_DIR,
    `events-backup-before-rallyes-coches-${formatTimestamp(new Date())}.json`,
  );
  const backupContent = JSON.stringify(
    {
      createdAt: new Date().toISOString(),
      source: "public.events",
      reason: "backup before non-destructive rallyes/coches import",
      count: events.length,
      events,
    },
    null,
...
function findDuplicateAgainstExisting(event: ValidSeedEvent, existingEvents: ExistingEvent[]): DuplicateCandidate | null {
  for (const existingEvent of existingEvents) {
    ...
    if (sameTitleDateAndPlace(event, existingEvent)) {
      return {
        event,
        matchedEvent: existingEvent,
        reason: "mismo title normalizado + start_date + city o province",
      };
    }

    if (sameTitleDateAndSourceUrl(event, existingEvent)) {
      return {
        event,
        matchedEvent: existingEvent,
        reason: "mismo source_url + mismo title normalizado + misma fecha",
      };
    }
  }

  for (const existingEvent of existingEvents) {
    if (!sameDateAndProvince(event, existingEvent)) {
      continue;
    }

    const similarity = titleSimilarity(event.title, existingEvent.title);

    if (similarity >= SIMILAR_TITLE_THRESHOLD && similarDiscipline(event, existingEvent)) {
      return {
        event,
        matchedEvent: existingEvent,
        reason: "title muy parecido + misma fecha + misma provincia + disciplina parecida",
        similarity,
      };
    }
  }
...
async function main() {
  ...
  console.log("Importacion segura de rallyes/coches 2026");
  ...
  printDuplicates("Duplicados internos que NO se importaran", duplicatesInSeed);
  ...
  console.log("\nResumen previo:");
  console.log(`- eventos leidos: ${seedEvents.length}`);
  console.log(`- eventos validos: ${validEvents.length}`);
  console.log(`- eventos invalidos: ${invalidEvents.length}`);
  console.log(`- duplicados internos: ${duplicatesInSeed.length}`);
  console.log(`- posibles duplicados contra Supabase: ${duplicatesAgainstExisting.length}`);
  console.log(`- eventos nuevos a insertar: ${newEvents.length}`);

  if (dryRun) {
    console.log("\nDry-run completado. No se ha insertado nada.");
    console.log("- eventos insertados: 0");
  ...
  const { data: insertedEvents, error: insertError } = await supabase.from("events").insert(rows).select("id");
...
main().catch((error: unknown) => {
  ...
  console.error(`\nImportacion de rallyes/coches fallida: ${message}`);
  process.exitCode = 1;
});
