...
const JSON_PATH = path.join(process.cwd(), "data", "eventos-concentraciones-moteras-espana-2026.json");
...
const VALID_CATEGORY_KEYS = new Set([
  ...
  "matinal-motera",
  "evento-biker",
  "concentracion-custom",
  "ruta-solidaria",
  "concentracion-solidaria",
  "encuentro-motero",
  "fiesta-motera",
]);
const VALID_VEHICLE_TYPES = new Set(["moto", "coche", "mixto", "karting", "otros"]);
const DOMAIN_TERMS = [
  "concentracion",
  "concentración",
  "motoalmuerzo",
...
function normalizeVehicleType(value, searchableText) {
  const explicitType = normalizeText(value);

  if (VALID_VEHICLE_TYPES.has(explicitType)) {
    return explicitType;
  }

  if (explicitType === "karting/otros" || explicitType === "karting-otros") {
    return "otros";
  }

  const text = normalizeText(searchableText);

  if (text.includes("moto") && (text.includes("coche") || text.includes("automovil"))) return "mixto";
  if (
    text.includes("moto") ||
    text.includes("motera") ||
    text.includes("motero") ||
    text.includes("biker") ||
    text.includes("custom") ||
    text.includes("motoclub")
  ) {
    return "moto";
  }
  if (text.includes("coche") || text.includes("automovil")) return "coche";
...
function normalizeTags(value, category, reasons) {
  ...
  for (const tag of [...tags, "concentracion", "moto", "Concentraciones", category].filter(Boolean)) {
...
function validateEvent(value, index) {
  ...
  const category = getString(value.category) || "Concentracion motera";
  const discipline = getString(value.discipline) || "Concentraciones";
  const rawVehicleType = getString(value.vehicle_type) || getString(value.vehicleType);
  const status = getString(value.status) || "published";
  ...
  if (normalizeText(discipline) !== "concentraciones") {
    reasons.push('discipline debe ser "Concentraciones"');
  }

  if (!VALID_CATEGORY_KEYS.has(normalizeComparable(category))) {
    reasons.push(`category no reconocida: ${category}`);
  }

  if (normalizeText(status) && !["published", "verified", "cancelled"].includes(normalizeText(status))) {
    reasons.push(`status no reconocido: ${status}`);
  }
  ...
  const searchableText = [
    ...
    getString(value.venue),
    getString(value.description),
    Array.isArray(value.tags) ? value.tags.join(" ") : "",
  ].join(" ");
  const vehicleType = normalizeVehicleType(rawVehicleType, searchableText);

  if (!["moto", "mixto"].includes(vehicleType)) {
    reasons.push(`vehicle_type no esperado para concentraciones moteras: ${rawVehicleType || vehicleType}`);
  }

  if (!includesAny(searchableText, DOMAIN_TERMS)) {
    reasons.push("el evento no parece realmente una concentracion motera, motoalmuerzo, quedada, matinal o encuentro biker");
  }
  ...
  if (reasons.length) {
    return {
      index,
      id: getString(value.id) || `(concentraciones-${slug || index + 1})`,
      title: title || "(sin titulo)",
      reasons,
    };
  }

  const id = getString(value.id) || `concentraciones-${slug}`;
  ...
  return {
    ...
    visible: normalizeText(status) !== "cancelled",
    import_method: "concentraciones-json",
...
function countBy(events, field) {
  return events.reduce((counts, event) => {
    const key = event[field] || "(sin valor)";
    counts[key] = (counts[key] || 0) + 1;
    return counts;
  }, {});
}

function printGrouping(title, counts) {
  console.log(`\n${title}:`);
...
async function main() {
  ...
  console.log("Importacion segura de eventos de concentraciones moteras 2026");
  ...
  printGrouping("Agrupacion por vehicle_type de eventos listos", countBy(readyToImport, "vehicle_type"));
  printGrouping("Agrupacion por category de eventos listos", countBy(readyToImport, "category"));
  printGrouping("Agrupacion por discipline de eventos listos", countBy(readyToImport, "discipline"));
  ...
  if (insertError) {
    console.error("\nError de Supabase al insertar eventos de concentraciones:");
