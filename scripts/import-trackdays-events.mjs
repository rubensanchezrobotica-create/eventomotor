...
const JSON_PATH = path.join(process.cwd(), "data", "eventos-trackdays-rodadas-espana-2026.json");
...
function normalizeVehicleType(value, searchableText) {
  ...
  if (text.includes("moto") && !text.includes("automovil") && !text.includes("coche")) return "moto";
  if (text.includes("drift") || text.includes("automovil") || text.includes("coche") || text.includes("gt")) return "coche";
  if (text.includes("trackday") || text.includes("rodada") || text.includes("tandas") || text.includes("circuito")) return "mixto";
...
function normalizeTags(value, category, reasons) {
  ...
  for (const tag of [...tags, "circuito", "Circuito", category].filter(Boolean)) {
...
function validateEvent(value, index) {
  ...
  const category = getString(value.category) || "Trackday";
  const discipline = getString(value.discipline) || "Circuito";
  ...
  if (normalizeText(discipline) !== "circuito") reasons.push('discipline debe ser "Circuito"');
  ...
  if (!VALID_VEHICLE_TYPES.has(vehicleType)) {
    reasons.push(`vehicle_type no compatible: ${rawVehicleType}`);
  }

  if (!includesAny(searchableText, DOMAIN_TERMS)) {
    reasons.push("el evento no parece realmente de trackdays, rodadas, tandas o experiencias en circuito");
  }
  ...
  if (reasons.length) {
    return {
      index,
      id: getString(value.id) || `(trackdays-${slug || index + 1})`,
      title: title || "(sin titulo)",
      reasons,
    };
  }

  const id = getString(value.id) || `trackdays-${slug}`;
...
async function writeBackup(events) {
  ...
  const backupPath = path.join(
    BACKUP_DIR,
    `events-backup-before-trackdays-${formatTimestamp(new Date())}.json`,
  );
  const backupContent = JSON.stringify(
    {
      createdAt: new Date().toISOString(),
      source: "public.events",
      reason: "backup before non-destructive trackdays import",
      count: events.length,
      events,
    },
    null,
...
async function main() {
  ...
  console.log("Importacion segura de eventos de trackdays y rodadas 2026");
  ...
  if (insertError) {
    console.error("\nError de Supabase al insertar eventos de trackdays:");
...
main().catch((error) => {
  ...
  console.error(`\nImportacion de trackdays fallida: ${message}`);
  process.exitCode = 1;
});
