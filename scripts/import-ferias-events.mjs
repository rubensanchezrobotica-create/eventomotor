...
const JSON_PATH = path.join(process.cwd(), "data", "eventos-ferias-motor-espana-2026.json");
...
function normalizeVehicleType(value, searchableText) {
  ...
  if (text.includes("automovil") || text.includes("coche") || text.includes("caravaning") || text.includes("camper")) return "coche";
  if (text.includes("motor") || text.includes("clasicos") || text.includes("feria")) return "mixto";
...
function normalizeTags(value, category, reasons) {
  ...
  for (const tag of [...tags, "feria", "Ferias", category].filter(Boolean)) {
...
function validateEvent(value, index) {
  ...
  const category = getString(value.category) || "Feria del motor";
  const discipline = getString(value.discipline) || "Ferias";
  ...
  if (normalizeText(discipline) !== "ferias") reasons.push('discipline debe ser "Ferias"');
  ...
  if (!includesAny(searchableText, FERIAS_TERMS)) {
    reasons.push("el evento no parece realmente de ferias, salones, exposiciones o eventos comerciales del motor");
  }
  ...
  if (reasons.length) {
    return {
      index,
      id: getString(value.id) || `(ferias-${slug || index + 1})`,
      title: title || "(sin titulo)",
      reasons,
    };
  }

  const id = getString(value.id) || `ferias-${slug}`;
...
async function writeBackup(events) {
  ...
  const backupPath = path.join(
    BACKUP_DIR,
    `events-backup-before-ferias-${formatTimestamp(new Date())}.json`,
  );
  const backupContent = JSON.stringify(
    {
      createdAt: new Date().toISOString(),
      source: "public.events",
      reason: "backup before non-destructive ferias import",
      count: events.length,
      events,
    },
    null,
...
async function main() {
  ...
  console.log("Importacion segura de eventos de ferias del motor 2026");
  ...
  if (insertError) {
    console.error("\nError de Supabase al insertar eventos de ferias:");
...
main().catch((error) => {
  ...
  console.error(`\nImportacion de ferias fallida: ${message}`);
  process.exitCode = 1;
});
