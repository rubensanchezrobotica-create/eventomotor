...
import {
  NEWSLETTER_EDITION_06_CAMPAIGN_ARMED_VALUE,
  NEWSLETTER_EDITION_06_CAMPAIGN_CONFIRM_PHRASE,
  ...
  sendPreparedNewsletterEdition06Campaign,
  type NewsletterEdition06CampaignClient,
  type NewsletterEdition06CampaignEnvironment,
  type NewsletterEdition06CampaignRequest,
  type NewsletterEdition06CampaignRepository,
  type NewsletterEdition06CampaignSummary,
...
import {
  ...
  NewsletterEdition06ContentError,
...
const PUBLIC_ASSET_DIRECTORY = "public/newsletter/2026-09-10/assets";
const UNSUBSCRIBE_URL =
  "https://www.eventomotor.com/newsletter/unsubscribe?token=edition06-preview-token-fixture-000000000000";
const EDITION_06_BODY_HTML_SHA256 =
  "5cce2cfec34aefcbfa91e371000030e4135069487858d0fd44c0192c1525b2f0";
const EDITION_06_HREFS_SHA256 =
  "29c5826dd5208eefc60f8b69c6ab781eca6c6021a1e6a48016df028c08483a0d";
const EDITION_06_EVENT_IMAGES_SHA256 =
  "837b3dc67258d28181530dc4171c50b10683d857491b89586efe86486e9e3d08";
const EDITION_06_HEADER_ASSET_SHA256 =
  "4163658063440544171d358a78b37ea0a559e2a7423d4aa9ea8ebce31d69b6bf";
const PREPARED_CAMPAIGN_ID = "12000000-0000-4000-8000-000000000006";
const PREPARED_CAMPAIGN_SEAL: NewsletterEdition06PreparedCampaignSeal = {
...
function summary(
  overrides: Partial<NewsletterEdition06CampaignSummary> = {},
...
function repositoryFixture(
  overrides: Partial<NewsletterEdition06CampaignRepository> = {},
  ...
  return {
    ...
    prepareCampaign: async () =>
      summary({
        ...
        audienceFrozenAt: "2026-09-10T08:00:00.000Z",
        preparedCount: 37,
      }),
...
function sendPreparedRequest(
  overrides: Partial<NewsletterEdition06CampaignRequest> = {},
): NewsletterEdition06CampaignRequest {
  return {
    sendPrepared: true,
    prepareOnly: false,
    limit: PREPARED_CAMPAIGN_SEAL.deliveryCount,
    confirmEdition: NEWSLETTER_EDITION_06_CAMPAIGN_KEY,
    confirmPhrase: NEWSLETTER_EDITION_06_CAMPAIGN_CONFIRM_PHRASE,
    confirmCampaignId: PREPARED_CAMPAIGN_ID,
    ...overrides,
  };
}

function sendPreparedEnvironment(): NewsletterEdition06CampaignEnvironment {
  return {
    armed: NEWSLETTER_EDITION_06_CAMPAIGN_ARMED_VALUE,
    apiKey: "re_test_edition06_prepared_campaign_only",
...
function preparedVariant(index: number): NewsletterEdition06ContentVariant {
...
test("Edition 06 has the approved independent identity", () => {
  assert.equal(NEWSLETTER_EDITION_06_CAMPAIGN_KEY, "agenda_motor_2026_09_10");
  assert.notEqual(NEWSLETTER_EDITION_06_CAMPAIGN_KEY, NEWSLETTER_EDITION_01_CAMPAIGN_KEY);
  assert.notEqual(NEWSLETTER_EDITION_06_CAMPAIGN_KEY, NEWSLETTER_EDITION_02_CAMPAIGN_KEY);
  assert.equal(
    NEWSLETTER_EDITION_06_SUBJECT,
  ...
  assert.equal(NEWSLETTER_EDITION_06_REPLY_TO, "info@eventomotor.com");
});

test("canonical HTML, text and seven assets pass fail-closed integrity", async () => {
  const source = await sourceFixture();
  assert.deepEqual(validateEdition06SourceIntegrity(source), {
    imageCount: 6,
    linkCount: 14,
  ...
  assert.match(NEWSLETTER_EDITION_06_HTML_SHA256, /^[0-9a-f]{64}$/);
  assert.match(NEWSLETTER_EDITION_06_TEXT_SHA256, /^[0-9a-f]{64}$/);

  const header = Buffer.from(source.assets["eventomotor-header.png"]);
  assert.equal(sha256(header), EDITION_06_HEADER_ASSET_SHA256);
  ...
  for (const asset of NEWSLETTER_EDITION_06_ASSET_MANIFEST) {
...
test("canonical content and asset changes fail closed", async () => {
  const source = await sourceFixture();
  assert.throws(
    () => validateEdition06SourceIntegrity({ ...source, html: `${source.html} ` }),
    (error) =>
      error instanceof NewsletterEdition06ContentError &&
      error.code === "template_digest_mismatch",
  );
  const changed = Uint8Array.from(source.assets["01-f1-madring-hero.webp"]);
  changed[0] = (changed[0] ?? 0) ^ 1;
  assert.throws(
    () =>
      validateEdition06SourceIntegrity({
        ...source,
        assets: { ...source.assets, "01-f1-madring-hero.webp": changed },
      }),
    (error) =>
      error instanceof NewsletterEdition06ContentError &&
...
test("all four technical variants render identical national content", async () => {
  const source = await sourceFixture();
  const national = prepareEdition06Content(source, "national", UNSUBSCRIBE_URL);
  const madrid = prepareEdition06Content(source, "madrid", UNSUBSCRIBE_URL);
  const aCoruna = prepareEdition06Content(source, "a-coruna", UNSUBSCRIBE_URL);
  const barcelona = prepareEdition06Content(source, "barcelona", UNSUBSCRIBE_URL);
  ...
  for (const rendered of [national, madrid, aCoruna, barcelona]) {
    ...
    assert.match(
      rendered.html,
      /Este fin de semana tiene un protagonista evidente, pero la agenda va bastante más allá de la Fórmula 1\./,
    );
    assert.match(rendered.html, />Fórmula 1 · Gran Premio de España · MADRING<\/h2>/);
    assert.match(
      rendered.html,
      /La Fórmula 1 estrena MADRING con actividad desde el viernes, clasificación el sábado y carrera el domingo a las 15:00\./,
    );
    assert.match(
      rendered.html,
      /Tras el aplazamiento de julio, la Baja España Aragón recupera una de las grandes citas internacionales/,
    );
    assert.match(
      rendered.html,
      /Big Twin vuelve al Pinar del Grao con rutas moteras, Bike Show, Market, conciertos y actividades durante cuatro días/,
    );
    assert.match(
      rendered.html,
      /Dos jornadas para una disciplina donde el espectáculo no está en la velocidad/,
    );
    assert.match(
      rendered.html,
      /17–20 septiembre<\/strong> · A Coruña/,
    );
    assert.match(rendered.text, /FÓRMULA 1 · GRAN PREMIO DE ESPAÑA · MADRING/);
    assert.match(rendered.text, /BAJA ESPAÑA ARAGÓN · FIA WBC/);
    assert.match(rendered.text, /XLI CONCENTRACIÓN BIG TWIN ESPAÑA/);
    assert.match(rendered.text, /CAMPEONATO DE ESPAÑA DE TRIAL · VIMIANZO/);
    assert.match(rendered.text, /10º REPSOL ECORALLY A CORUÑA/);
    assert.match(
      rendered.text,
      /17–20 septiembre · A Coruña/,
    );
    assert.doesNotMatch(
      rendered.text,
...
test("Edition 06 reuses the sealed Edition 05 visual shell and 42/58 cards", async () => {
  const source = await sourceFixture();
  const national = prepareEdition06Content(source, "national", UNSUBSCRIBE_URL).html;
  const barcelona = prepareEdition06Content(source, "barcelona", UNSUBSCRIBE_URL).html;
  const imageColumn =
  ...
  assert.match(
    national,
    /alt="EventoMotor · La Agenda Motor · Edición 06 · 11–13 septiembre 2026"/,
  );
  assert.doesNotMatch(national, /eventomotor-logo\.png/);
  assert.match(national, /La selección de esta semana/);
  assert.match(
    national,
    /01-f1-madring-hero\.webp" width="564"[^>]+border-radius:14px;/,
  );
  assert.match(national, /<div style="padding:18px 2px 0;">/);
  assert.match(national, /Evento destacado · Fórmula 1/);
  ...
  assert.match(
    national,
    /Reserva fecha<\/div>[\s\S]*10º REPSOL EcoRally A Coruña/,
  );
  assert.doesNotMatch(
    national,
  ...
  const orderedTitles = [
    ...
    "Baja España Aragón · FIA WBC",
    "XLI Concentración Big Twin España",
    "Campeonato de España de Trial · Vimianzo",
...
test("Gmail iPhone header and footer reuse the historical opaque email-safe hardening", async () => {
  ...
  assert.equal(sha256(editorialBody(source.html)), EDITION_06_BODY_HTML_SHA256);
  const hrefs = [...source.html.matchAll(/<a\b[^>]*\bhref="([^"]+)"/g)].map(
    (match) => match[1] ?? "",
  );
  assert.equal(sha256(JSON.stringify(hrefs)), EDITION_06_HREFS_SHA256);
  const eventImages = [
  ...
  assert.equal(
    ...
    EDITION_06_EVENT_IMAGES_SHA256,
  );
  assert.equal(
...
test("all four previews are exact outputs of the runtime renderer", async () => {
  ...
  for (const variant of variants) {
    const expected = prepareEdition06PreviewContent(source, variant, UNSUBSCRIBE_URL);
...
test("dry-run is aggregate-only and constructs no provider client", async () => {
  ...
  let prepared = 0;
  let clients = 0;
  const result = await executeNewsletterEdition06Campaign({
...
test("mutation modes retain Edition 06-specific fail-closed gates", async () => {
  const source = await sourceFixture();
  await assert.rejects(
    ...
    (error) =>
      error instanceof NewsletterEdition06CampaignError &&
...
test("send-prepared consumes exactly the frozen deliveries without preview or prepare", async () => {
  ...
  const repository = repositoryFixture({
    ...
    claimDelivery: async (input) => {
      ...
      return {
        ...
        recipientEmail: `edition06-${index}@example.invalid`,
  ...
  const client: NewsletterEdition06CampaignClient = {
    sendEmail: async (payload) => {
      providerCalls += 1;
      payloads.push(payload);
      return {
        status: "accepted",
        providerMessageId: `edition06-prepared-${providerCalls}`,
      };
    },
  };

  const result = await sendPreparedNewsletterEdition06Campaign({
    request: sendPreparedRequest(),
    environment: sendPreparedEnvironment(),
    source,
    repository,
  ...
  assert.equal(payloads[0]?.subject, NEWSLETTER_EDITION_06_SUBJECT);
  assert.match(payloads[0]?.html ?? "", /utm_campaign=agenda_motor_2026_09_10/);
...
test("campaign parser is dry-run by default and rejects unsafe combinations", () => {
  assert.deepEqual(parseNewsletterEdition06CampaignArguments([]), {
    sendPrepared: false,
    prepareOnly: false,
    limit: 25,
  });
  assert.throws(
    () => parseNewsletterEdition06CampaignArguments(["--resume"]),
    (error) =>
      error instanceof NewsletterEdition06CampaignError &&
      error.code === "unknown_argument",
  );
  assert.throws(
    () => parseNewsletterEdition06CampaignArguments(["--send"]),
    (error) =>
      error instanceof NewsletterEdition06CampaignError &&
      error.code === "unknown_argument",
  );
...
test("server boundaries remain explicit and the CLI imports only the server adapter", async () => {
  const [serverAdapter, repositoryAdapter, sourceAdapter, cli] = await Promise.all([
    readFile(resolve(ROOT, "lib/newsletter/edition-06-campaign.server.ts"), "utf8"),
    readFile(resolve(ROOT, "lib/newsletter/edition-06-campaign-repository.server.ts"), "utf8"),
    readFile(resolve(ROOT, "lib/newsletter/edition-06-source.server.ts"), "utf8"),
    readFile(resolve(ROOT, "scripts/send-newsletter-edition-06.ts"), "utf8"),
  ]);
  for (const adapter of [serverAdapter, repositoryAdapter, sourceAdapter]) {
    assert.ok(adapter.startsWith('import "server-only";'));
  }
  assert.match(cli, /edition-06-campaign\.server/);
  assert.doesNotMatch(cli, /resend-client|supabase|service-role/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_CAMPAIGN_ID/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_DELIVERY_COUNT/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_NATIONAL_COUNT/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_MADRID_COUNT/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_A_CORUNA_COUNT/);
  assert.match(serverAdapter, /NEWSLETTER_EDITION_06_PREPARED_BARCELONA_COUNT/);
});
