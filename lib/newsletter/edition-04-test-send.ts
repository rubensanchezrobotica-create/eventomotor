import {
  ...
  type NewsletterEdition04TemplateSummary,
...
export const NEWSLETTER_EDITION_04_TEST_ARMED_VALUE =
  NEWSLETTER_EDITION_01_TEST_ARMED_VALUE;
export const NEWSLETTER_EDITION_04_TEST_CONFIRM_PHRASE =
  "SEND-EDITION-04-TEST";
export const NEWSLETTER_EDITION_04_SAFE_TEST_UNSUBSCRIBE_URL =
...
export type NewsletterEdition04TestRequest = {
  send: boolean;
  to?: string;
  confirmTo?: string;
...
export type NewsletterEdition04TestEnvironment = {
  armed?: string;
  apiKey?: string;
  ci?: string;
...
export type NewsletterEdition04TestEmailPayload = {
  from: string;
  to: readonly [string];
  replyTo: string;
  subject: string;
...
export type NewsletterEdition04TestClientResult =
  | { status: "accepted"; providerMessageId: string }
  | { status: "provider_error"; httpStatus: number | null }
  | { status: "timeout" }
  | { status: "invalid_response"; httpStatus: number };

export interface NewsletterEdition04TestClient {
  sendEmail(
    payload: NewsletterEdition04TestEmailPayload,
  ): Promise<NewsletterEdition04TestClientResult>;
}

type NewsletterEdition04TestResultBase = {
  recipient: string;
  variant: "national";
  subject: string;
  summary: NewsletterEdition04TemplateSummary;
  allowlistCount: 1;
};

export type NewsletterEdition04TestResult =
  | (NewsletterEdition04TestResultBase & {
      status: "dry_run";
...
export type ExecuteNewsletterEdition04TestOptions = {
  request: NewsletterEdition04TestRequest;
  source: NewsletterEdition04Source;
  sender: string;
  replyTo: string;
  environment?: NewsletterEdition04TestEnvironment;
  clientFactory?: (apiKey: string) => NewsletterEdition04TestClient;
  logger?: (message: string) => void;
};

export class NewsletterEdition04TestSendError extends Error {
  constructor(readonly code: string) {
    super(`Edition 04 test send blocked: ${code}.`);
    this.name = "NewsletterEdition04TestSendError";
  }
}

function fail(code: string): never {
  throw new NewsletterEdition04TestSendError(code);
}
...
function validatedVariant(
  value: NewsletterEdition04ContentVariant | undefined,
): "national" {
...
function validatedTestUnsubscribeUrl(value: string | undefined): string {
  const candidate = value ?? NEWSLETTER_EDITION_04_SAFE_TEST_UNSUBSCRIBE_URL;
  if (candidate !== NEWSLETTER_EDITION_04_SAFE_TEST_UNSUBSCRIBE_URL) {
...
function assertSendGates(
  ...
  if (request.confirmPhrase !== NEWSLETTER_EDITION_04_TEST_CONFIRM_PHRASE) {
    fail("confirmation_phrase_invalid");
  }
  if (environment.armed !== NEWSLETTER_EDITION_04_TEST_ARMED_VALUE) {
    fail("send_not_armed");
  }
  if (
    !environment.apiKey ||
...
function logSummary(
  ...
  logger("EDITION_04_PROTECTED_TEST_VALIDATION=PASS");
  ...
  logger(
    `TEST_SEND_ARMED=${
      environment.armed === NEWSLETTER_EDITION_04_TEST_ARMED_VALUE ? "YES" : "NO"
    }`,
  );
...
export async function executeNewsletterEdition04TestSend(
  ...
  const prepared = prepareEdition04Content(
    options.source,
    variant,
    unsubscribeUrl,
  );
...
export function parseNewsletterEdition04TestArguments(
  argv: readonly string[],
): NewsletterEdition04TestRequest {
  const request: NewsletterEdition04TestRequest = {
    send: false,
    variant: "national",
  };
