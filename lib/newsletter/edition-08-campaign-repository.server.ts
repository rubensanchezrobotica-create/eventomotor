...
import type {
  NewsletterEdition08CampaignClaim,
  NewsletterEdition08CampaignIdentity,
  NewsletterEdition08CampaignRepository,
...
type SummaryRow =
  Functions["preview_newsletter_campaign_edition_08"]["Returns"][number];
...
export interface NewsletterEdition08CampaignRpcGateway {
  previewCampaign(
    args: Functions["preview_newsletter_campaign_edition_08"]["Args"],
  ): Promise<RpcResult<SummaryRow>>;
  prepareCampaign(
    args: Functions["prepare_newsletter_campaign_edition_08"]["Args"],
  ): Promise<RpcResult<SummaryRow>>;
  claimDelivery(
...
function persistenceFailure(): Error {
  return new Error("Newsletter edition 08 persistence operation failed safely.");
}
...
function summaryFromRow(row: SummaryRow): NewsletterEdition08CampaignSummary {
  if (
  ...
  return {
    ...
    campaignStatus:
      row.campaign_status as NewsletterEdition08CampaignSummary["campaignStatus"],
    ...
    catalunaCount: row.cataluna_count,
    comunidadValencianaCount: row.comunidad_valenciana_count,
...
function identityArgs(identity: NewsletterEdition08CampaignIdentity) {
  return {
...
export function createNewsletterEdition08CampaignRpcGateway(
  client: SupabaseClient<Database>,
): NewsletterEdition08CampaignRpcGateway {
  return {
...
export function createNewsletterEdition08CampaignRepository(
  gateway: NewsletterEdition08CampaignRpcGateway,
): NewsletterEdition08CampaignRepository {
  return {
    ...
    async claimDelivery(input) {
      ...
      const claim: NewsletterEdition08CampaignClaim = {
        ...
        contentVariant:
          row.content_variant as NewsletterEdition08CampaignClaim["contentVariant"],
      };
      return claim;
    },
...
export function createConfiguredNewsletterEdition08CampaignRepository(): NewsletterEdition08CampaignRepository | null {
  const client = createSupabaseServerClient();
  return client
    ? createNewsletterEdition08CampaignRepository(
        createNewsletterEdition08CampaignRpcGateway(client),
      )
    : null;
}
