import {
  ...
  type NewsletterEdition06TemplateSummary,
...
export const NEWSLETTER_EDITION_06_TEST_ARMED_VALUE =
  NEWSLETTER_EDITION_01_TEST_ARMED_VALUE;
export const NEWSLETTER_EDITION_06_TEST_CONFIRM_PHRASE =
  "SEND-EDITION-06-TEST";
export const NEWSLETTER_EDITION_06_SAFE_TEST_UNSUBSCRIBE_URL =
...
export type NewsletterEdition06TestRequest = {
  send: boolean;
  to?: string;
  confirmTo?: string;
...
export type NewsletterEdition06TestEnvironment = {
  armed?: string;
  apiKey?: string;
  ci?: string;
...
export type NewsletterEdition06TestEmailPayload = {
  from: string;
  to: readonly [string];
  replyTo: string;
  subject: string;
...
export type NewsletterEdition06TestClientResult =
  | { status: "accepted"; providerMessageId: string }
  | { status: "provider_error"; httpStatus: number | null }
  | { status: "timeout" }
  | { status: "invalid_response"; httpStatus: number };

export interface NewsletterEdition06TestClient {
  sendEmail(
    payload: NewsletterEdition06TestEmailPayload,
  ): Promise<NewsletterEdition06TestClientResult>;
}

type NewsletterEdition06TestResultBase = {
  recipient: string;
  variant: "national";
  subject: string;
  summary: NewsletterEdition06TemplateSummary;
  allowlistCount: 1;
};

export type NewsletterEdition06TestResult =
  | (NewsletterEdition06TestResultBase & {
      status: "dry_run";
...
export type ExecuteNewsletterEdition06TestOptions = {
  request: NewsletterEdition06TestRequest;
  source: NewsletterEdition06Source;
  sender: string;
  replyTo: string;
  environment?: NewsletterEdition06TestEnvironment;
  clientFactory?: (apiKey: string) => NewsletterEdition06TestClient;
  logger?: (message: string) => void;
};

export class NewsletterEdition06TestSendError extends Error {
  constructor(readonly code: string) {
    super(`Edition 06 test send blocked: ${code}.`);
    this.name = "NewsletterEdition06TestSendError";
  }
}

function fail(code: string): never {
  throw new NewsletterEdition06TestSendError(code);
}
...
function validatedVariant(
  value: NewsletterEdition06ContentVariant | undefined,
): "national" {
...
function validatedTestUnsubscribeUrl(value: string | undefined): string {
  const candidate = value ?? NEWSLETTER_EDITION_06_SAFE_TEST_UNSUBSCRIBE_URL;
  if (candidate !== NEWSLETTER_EDITION_06_SAFE_TEST_UNSUBSCRIBE_URL) {
...
function assertSendGates(
  ...
  if (request.confirmPhrase !== NEWSLETTER_EDITION_06_TEST_CONFIRM_PHRASE) {
    fail("confirmation_phrase_invalid");
  }
  if (environment.armed !== NEWSLETTER_EDITION_06_TEST_ARMED_VALUE) {
    fail("send_not_armed");
  }
  if (
    !environment.apiKey ||
...
function logSummary(
  ...
  logger("EDITION_06_PROTECTED_TEST_VALIDATION=PASS");
  ...
  logger(
    `TEST_SEND_ARMED=${
      environment.armed === NEWSLETTER_EDITION_06_TEST_ARMED_VALUE ? "YES" : "NO"
    }`,
  );
...
export async function executeNewsletterEdition06TestSend(
  ...
  const prepared = prepareEdition06Content(
    options.source,
    variant,
    unsubscribeUrl,
  );
...
export function parseNewsletterEdition06TestArguments(
  argv: readonly string[],
): NewsletterEdition06TestRequest {
  const request: NewsletterEdition06TestRequest = {
    send: false,
    variant: "national",
  };
