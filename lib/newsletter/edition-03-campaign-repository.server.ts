...
import type {
  NewsletterEdition03CampaignClaim,
  NewsletterEdition03CampaignIdentity,
...
export interface NewsletterEdition03CampaignRpcGateway {
  previewCampaign(
...
function persistenceFailure(): Error {
  return new Error("Newsletter edition 03 persistence operation failed safely.");
}
...
function summaryFromRow(row: SummaryRow): NewsletterEdition03CampaignSummary {
  if (
  ...
  return {
    ...
    campaignStatus:
      row.campaign_status as NewsletterEdition03CampaignSummary["campaignStatus"],
...
function identityArgs(identity: NewsletterEdition03CampaignIdentity) {
  return {
...
export function createNewsletterEdition03CampaignRpcGateway(
  client: SupabaseClient<Database>,
): NewsletterEdition03CampaignRpcGateway {
  return {
...
export function createNewsletterEdition03CampaignRepository(
  gateway: NewsletterEdition03CampaignRpcGateway,
): NewsletterEdition03CampaignRepository {
  return {
    ...
    async claimDelivery(input) {
      ...
      const claim: NewsletterEdition03CampaignClaim = {
...
export function createConfiguredNewsletterEdition03CampaignRepository(): NewsletterEdition03CampaignRepository | null {
  const client = createSupabaseServerClient();
  return client
    ? createNewsletterEdition03CampaignRepository(
        createNewsletterEdition03CampaignRpcGateway(client),
      )
    : null;
}
