...
import type {
  NewsletterEdition05CampaignClaim,
  NewsletterEdition05CampaignIdentity,
...
export interface NewsletterEdition05CampaignRpcGateway {
  previewCampaign(
...
function persistenceFailure(): Error {
  return new Error("Newsletter edition 05 persistence operation failed safely.");
}
...
function summaryFromRow(row: SummaryRow): NewsletterEdition05CampaignSummary {
  if (
  ...
  return {
    ...
    campaignStatus:
      row.campaign_status as NewsletterEdition05CampaignSummary["campaignStatus"],
...
function identityArgs(identity: NewsletterEdition05CampaignIdentity) {
  return {
...
export function createNewsletterEdition05CampaignRpcGateway(
  client: SupabaseClient<Database>,
): NewsletterEdition05CampaignRpcGateway {
  return {
...
export function createNewsletterEdition05CampaignRepository(
  gateway: NewsletterEdition05CampaignRpcGateway,
): NewsletterEdition05CampaignRepository {
  return {
    ...
    async claimDelivery(input) {
      ...
      const claim: NewsletterEdition05CampaignClaim = {
...
export function createConfiguredNewsletterEdition05CampaignRepository(): NewsletterEdition05CampaignRepository | null {
  const client = createSupabaseServerClient();
  return client
    ? createNewsletterEdition05CampaignRepository(
        createNewsletterEdition05CampaignRpcGateway(client),
      )
    : null;
}
