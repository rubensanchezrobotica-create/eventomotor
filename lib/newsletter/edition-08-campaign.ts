...
export const NEWSLETTER_EDITION_08_CAMPAIGN_CONFIRM_PHRASE =
  "SEND-AGENDA-MOTOR-2026-09-24";
export const NEWSLETTER_EDITION_08_CAMPAIGN_ARMED_VALUE =
  "agenda-motor-2026-09-24-manual-send";
export const NEWSLETTER_EDITION_08_UNSUBSCRIBE_ORIGIN =
...
export type NewsletterEdition08CampaignRequest = {
...
export type NewsletterEdition08PreparedCampaignSeal = {
  campaignId: string;
  deliveryCount: number;
  variantCounts: Readonly<Record<NewsletterEdition08ContentVariant, number>>;
};
...
export const NEWSLETTER_EDITION_08_PREPARED_CAMPAIGN_SEAL: NewsletterEdition08PreparedCampaignSeal | null =
  null;

export type NewsletterEdition08CampaignEnvironment = {
  armed?: string;
  apiKey?: string;
  ci?: string;
...
export type NewsletterEdition08CampaignIdentity = {
  editionKey: string;
  subject: string;
  htmlSha256: string;
  textSha256: string;
...
export type NewsletterEdition08CampaignSummary = {
...
export type NewsletterEdition08CampaignClaim = {
  deliveryId: string;
  campaignId: string;
  subscriberId: string;
...
export interface NewsletterEdition08CampaignRepository {
  previewCampaign(
    identity: NewsletterEdition08CampaignIdentity,
  ): Promise<NewsletterEdition08CampaignSummary>;
  prepareCampaign(
    identity: NewsletterEdition08CampaignIdentity,
  ): Promise<NewsletterEdition08CampaignSummary>;
  claimDelivery(input: {
    ...
    tokenHash: string;
    allowRetry: boolean;
  }): Promise<NewsletterEdition08CampaignClaim | null>;
...
export type NewsletterEdition08CampaignEmailPayload = {
  from: string;
  to: readonly [string];
  replyTo: string;
  subject: string;
...
export type NewsletterEdition08CampaignClientResult =
  | { status: "accepted"; providerMessageId: string }
  | { status: "provider_error"; httpStatus: number | null }
  | { status: "timeout" }
  | { status: "invalid_response"; httpStatus: number };

export interface NewsletterEdition08CampaignClient {
  sendEmail(
    payload: NewsletterEdition08CampaignEmailPayload,
  ): Promise<NewsletterEdition08CampaignClientResult>;
}

export type ExecuteNewsletterEdition08CampaignOptions = {
  ...
  repository: NewsletterEdition08CampaignRepository;
  sender: string;
  replyTo: string;
  clientFactory?: (apiKey: string) => NewsletterEdition08CampaignClient;
  tokenFactory?: () => string;
  tokenHasher?: (token: string) => string;
  now?: () => Date;
  logger?: (message: string) => void;
  preparedCampaignSeal?: NewsletterEdition08PreparedCampaignSeal | null;
};

export type NewsletterEdition08CampaignResult = {
  status: "dry_run" | "prepared" | "prepared_sent";
  identity: NewsletterEdition08CampaignIdentity;
  digest: string;
  processedCount: number;
  summary: NewsletterEdition08CampaignSummary | null;
  campaignId?: string;
  processedVariantCounts?: Readonly<Record<NewsletterEdition08ContentVariant, number>>;
};

export class NewsletterEdition08CampaignError extends Error {
  constructor(readonly code: string) {
    super(`Edition 08 campaign blocked: ${code}.`);
    this.name = "NewsletterEdition08CampaignError";
  }
}

function fail(code: string): never {
  throw new NewsletterEdition08CampaignError(code);
}
...
export function newsletterEdition08CampaignIdentity(): NewsletterEdition08CampaignIdentity {
  return {
...
export function newsletterEdition08CampaignDigest(
  identity: NewsletterEdition08CampaignIdentity,
): string {
  return sha256(
    [
...
function assertMutationGates(
  ...
  if (request.confirmEdition !== NEWSLETTER_EDITION_08_CAMPAIGN_KEY) {
    fail("edition_confirmation_invalid");
  }
  if (request.confirmPhrase !== NEWSLETTER_EDITION_08_CAMPAIGN_CONFIRM_PHRASE) {
    fail("confirmation_phrase_invalid");
  }
  if (environment.armed !== NEWSLETTER_EDITION_08_CAMPAIGN_ARMED_VALUE) {
    fail("send_not_armed");
  }
...
function assertRequestMode(request: NewsletterEdition08CampaignRequest): void {
...
function assertPreparedCampaignSeal(
  seal: NewsletterEdition08PreparedCampaignSeal | null,
): asserts seal is NewsletterEdition08PreparedCampaignSeal {
...
function assertSummary(summary: NewsletterEdition08CampaignSummary): void {
  const values = [
...
function assertClaim(claim: NewsletterEdition08CampaignClaim): void {
  if (
    ...
    !["national", "madrid", "cataluna", "comunidad-valenciana"].includes(
...
function unsubscribeUrl(rawToken: string): string {
  const url = new URL(
    ...
    NEWSLETTER_EDITION_08_UNSUBSCRIBE_ORIGIN,
  );
  url.searchParams.set("token", rawToken);
  return url.toString();
}

function safeProviderFailure(result: NewsletterEdition08CampaignClientResult): {
  errorCode: string;
  retryable: boolean;
  unknown: boolean;
} {
...
function logSummary(
  ...
  identity: NewsletterEdition08CampaignIdentity,
  digest: string,
  summary: NewsletterEdition08CampaignSummary,
): void {
...
async function recordUnknownAndStop(
  repository: NewsletterEdition08CampaignRepository,
  claim: NewsletterEdition08CampaignClaim,
  errorCode: string,
  occurredAt: string,
): Promise<never> {
  try {
...
type NewsletterEdition08ProcessedBatch = {
  processedCount: number;
  variantCounts: Record<NewsletterEdition08ContentVariant, number>;
};

async function sendClaimedNewsletterEdition08Deliveries(
  options: ExecuteNewsletterEdition08CampaignOptions,
  campaignId: string,
  apiKey: string,
): Promise<NewsletterEdition08ProcessedBatch> {
  ...
  const variantCounts: Record<NewsletterEdition08ContentVariant, number> = {
    national: 0,
    madrid: 0,
    cataluna: 0,
  ...
  while (processedCount < options.request.limit) {
    ...
    const content = prepareEdition08Content(
      options.source,
      claim.contentVariant,
      unsubscribeUrl(rawToken),
    );
    const payload: NewsletterEdition08CampaignEmailPayload = {
      from: options.sender,
    ...
    let providerResult: NewsletterEdition08CampaignClientResult | null = null;
    try {
...
export async function sendPreparedNewsletterEdition08Campaign(
  options: ExecuteNewsletterEdition08CampaignOptions,
): Promise<NewsletterEdition08CampaignResult> {
  const environment = options.environment ?? {};
  const logger = options.logger ?? (() => undefined);
  const identity = newsletterEdition08CampaignIdentity();
  const digest = newsletterEdition08CampaignDigest(identity);
  ...
  validateEdition08SourceIntegrity(options.source);
  ...
  const preparedCampaignSeal =
    options.preparedCampaignSeal ?? NEWSLETTER_EDITION_08_PREPARED_CAMPAIGN_SEAL;
  ...
  const batch = await sendClaimedNewsletterEdition08Deliveries(
    options,
  ...
  logger(`Processed variant cataluna: ${batch.variantCounts.cataluna}`);
  logger(
    `Processed variant comunidad-valenciana: ${batch.variantCounts["comunidad-valenciana"]}`,
  );
...
export async function executeNewsletterEdition08Campaign(
  ...
  if (options.request.sendPrepared === true) {
    return sendPreparedNewsletterEdition08Campaign(options);
  }
  const environment = options.environment ?? {};
  const logger = options.logger ?? (() => undefined);
  const identity = newsletterEdition08CampaignIdentity();
...
export function parseNewsletterEdition08CampaignArguments(
  argv: readonly string[],
): NewsletterEdition08CampaignRequest {
  const request: NewsletterEdition08CampaignRequest = {
    sendPrepared: false,
    prepareOnly: false,
    limit: DEFAULT_LIMIT,
  };
