...
import type {
  NewsletterEdition07CampaignClaim,
  NewsletterEdition07CampaignIdentity,
...
export interface NewsletterEdition07CampaignRpcGateway {
  previewCampaign(
...
function summaryFromRow(row: SummaryRow): NewsletterEdition07CampaignSummary {
  if (
  ...
  return {
    ...
    campaignStatus:
      row.campaign_status as NewsletterEdition07CampaignSummary["campaignStatus"],
...
function identityArgs(identity: NewsletterEdition07CampaignIdentity) {
  return {
...
export function createNewsletterEdition07CampaignRpcGateway(
  client: SupabaseClient<Database>,
): NewsletterEdition07CampaignRpcGateway {
  return {
...
export function createNewsletterEdition07CampaignRepository(
  gateway: NewsletterEdition07CampaignRpcGateway,
): NewsletterEdition07CampaignRepository {
  return {
    ...
    async claimDelivery(input) {
      ...
      const claim: NewsletterEdition07CampaignClaim = {
...
export function createConfiguredNewsletterEdition07CampaignRepository(): NewsletterEdition07CampaignRepository | null {
  const client = createSupabaseServerClient();
  return client
    ? createNewsletterEdition07CampaignRepository(
        createNewsletterEdition07CampaignRpcGateway(client),
      )
    : null;
}
