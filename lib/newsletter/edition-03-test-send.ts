import {
  ...
  type NewsletterEdition03TemplateSummary,
...
export const NEWSLETTER_EDITION_03_TEST_ARMED_VALUE =
  NEWSLETTER_EDITION_01_TEST_ARMED_VALUE;
export const NEWSLETTER_EDITION_03_TEST_CONFIRM_PHRASE =
  "SEND-EDITION-03-TEST";
export const NEWSLETTER_EDITION_03_SAFE_TEST_UNSUBSCRIBE_URL =
...
export type NewsletterEdition03TestRequest = {
  send: boolean;
  to?: string;
  confirmTo?: string;
...
export type NewsletterEdition03TestEnvironment = {
  armed?: string;
  apiKey?: string;
  ci?: string;
...
export type NewsletterEdition03TestEmailPayload = {
  from: string;
  to: readonly [string];
  replyTo: string;
  subject: string;
...
export type NewsletterEdition03TestClientResult =
  | { status: "accepted"; providerMessageId: string }
  | { status: "provider_error"; httpStatus: number | null }
  | { status: "timeout" }
  | { status: "invalid_response"; httpStatus: number };

export interface NewsletterEdition03TestClient {
  sendEmail(
    payload: NewsletterEdition03TestEmailPayload,
  ): Promise<NewsletterEdition03TestClientResult>;
}

type NewsletterEdition03TestResultBase = {
  recipient: string;
  variant: "national";
  subject: string;
  summary: NewsletterEdition03TemplateSummary;
  allowlistCount: 1;
};

export type NewsletterEdition03TestResult =
  | (NewsletterEdition03TestResultBase & {
      status: "dry_run";
...
export type ExecuteNewsletterEdition03TestOptions = {
  request: NewsletterEdition03TestRequest;
  source: NewsletterEdition03Source;
  sender: string;
  replyTo: string;
  environment?: NewsletterEdition03TestEnvironment;
  clientFactory?: (apiKey: string) => NewsletterEdition03TestClient;
  logger?: (message: string) => void;
};

export class NewsletterEdition03TestSendError extends Error {
  constructor(readonly code: string) {
    super(`Edition 03 test send blocked: ${code}.`);
    this.name = "NewsletterEdition03TestSendError";
  }
}

function fail(code: string): never {
  throw new NewsletterEdition03TestSendError(code);
}
...
function validatedVariant(
  value: NewsletterEdition03ContentVariant | undefined,
): "national" {
...
function validatedTestUnsubscribeUrl(value: string | undefined): string {
  const candidate = value ?? NEWSLETTER_EDITION_03_SAFE_TEST_UNSUBSCRIBE_URL;
  if (candidate !== NEWSLETTER_EDITION_03_SAFE_TEST_UNSUBSCRIBE_URL) {
...
function assertSendGates(
  ...
  if (request.confirmPhrase !== NEWSLETTER_EDITION_03_TEST_CONFIRM_PHRASE) {
    fail("confirmation_phrase_invalid");
  }
  if (environment.armed !== NEWSLETTER_EDITION_03_TEST_ARMED_VALUE) {
    fail("send_not_armed");
  }
  if (
    !environment.apiKey ||
...
function logSummary(
  ...
  logger("EDITION_03_PROTECTED_TEST_VALIDATION=PASS");
  ...
  logger(
    `TEST_SEND_ARMED=${
      environment.armed === NEWSLETTER_EDITION_03_TEST_ARMED_VALUE ? "YES" : "NO"
    }`,
  );
...
export async function executeNewsletterEdition03TestSend(
  ...
  const prepared = prepareEdition03Content(
    options.source,
    variant,
    unsubscribeUrl,
  );
...
export function parseNewsletterEdition03TestArguments(
  argv: readonly string[],
): NewsletterEdition03TestRequest {
  const request: NewsletterEdition03TestRequest = {
    send: false,
    variant: "national",
  };
