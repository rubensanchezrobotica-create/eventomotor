...
import type {
  NewsletterEdition04CampaignClaim,
  NewsletterEdition04CampaignIdentity,
...
export interface NewsletterEdition04CampaignRpcGateway {
  previewCampaign(
...
function persistenceFailure(): Error {
  return new Error("Newsletter edition 04 persistence operation failed safely.");
}
...
function summaryFromRow(row: SummaryRow): NewsletterEdition04CampaignSummary {
  if (
  ...
  return {
    ...
    campaignStatus:
      row.campaign_status as NewsletterEdition04CampaignSummary["campaignStatus"],
...
function identityArgs(identity: NewsletterEdition04CampaignIdentity) {
  return {
...
export function createNewsletterEdition04CampaignRpcGateway(
  client: SupabaseClient<Database>,
): NewsletterEdition04CampaignRpcGateway {
  return {
...
export function createNewsletterEdition04CampaignRepository(
  gateway: NewsletterEdition04CampaignRpcGateway,
): NewsletterEdition04CampaignRepository {
  return {
    ...
    async claimDelivery(input) {
      ...
      const claim: NewsletterEdition04CampaignClaim = {
...
export function createConfiguredNewsletterEdition04CampaignRepository(): NewsletterEdition04CampaignRepository | null {
  const client = createSupabaseServerClient();
  return client
    ? createNewsletterEdition04CampaignRepository(
        createNewsletterEdition04CampaignRpcGateway(client),
      )
    : null;
}
